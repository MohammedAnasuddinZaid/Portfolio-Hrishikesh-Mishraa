    ...
    <>
      {!isOpen && (
        <motion.button
          ...
          className="fixed bottom-6 right-4 sm:right-6 z-[180] w-14 h-14 bg-[#0c0c0d] text-[#fcfcfc] flex items-center justify-center shadow-xl hover:bg-[#b0342e] transition-all cursor-pointer border border-[#fcfcfc]/10"
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className={`fixed bottom-6 right-4 sm:right-6 z-[180] w-[420px] max-w-[calc(100vw-2rem)] bg-[#0c0c0d] border border-[#fcfcfc]/10 shadow-2xl overflow-hidden ${isMinimized ? 'h-12' : 'h-[500px] max-h-[calc(100dvh-5rem)]'}`}
            style={{ borderRadius: '12px', bottom: 'calc(1.5rem + env(safe-area-inset-bottom))' }}
          >
            <div
              className="flex items-center justify-between px-4 py-2.5 bg-[#1a1813] border-b border-[#fcfcfc]/10 cursor-pointer select-none"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-mono text-[#fcfcfc]/60 ml-2 uppercase tracking-wider font-black">AI Terminal — Ctrl+K</span>
              </div>
              <button
                onClick={(e) => { e.stopPropagation(); setIsOpen(false); }}
                className="text-[#fcfcfc]/40 hover:text-[#fcfcfc] text-sm p-1"
              >
            ...
            {!isMinimized && (
              <div className="flex flex-col h-[calc(100%-44px)] font-mono text-sm">
                <div className="flex-1 overflow-y-auto p-4 space-y-1.5 custom-scrollbar">
                  {messages.map((msg, i) => (
                    <div key={i} className="leading-relaxed">
                      {msg.isCommand ? (
                        <span className="text-[#8a7b4f]">
                          <span className="text-[#b0342e]">❯</span> {msg.text.slice(2)}
                        </span>
                      ) : (
                        <span className="text-[#fcfcfc]/80 whitespace-pre-wrap">
                          {msg.text}
                          {msg.isTyping && <span className="inline-block w-2 h-4 bg-[#b0342e] ml-0.5 animate-pulse" />}
                        </span>
                      )}
                    </div>
                  ))}
                ...
                <div className="px-4 pb-2 flex flex-wrap gap-1.5">
                  {quickQuestions.map(qq => (
                    <button
                      ...
                      className="px-2 py-1 text-[9px] font-black uppercase tracking-wider border border-[#fcfcfc]/15 text-[#fcfcfc]/50 hover:text-[#fcfcfc] hover:border-[#b0342e]/50 hover:bg-[#b0342e]/10 transition-all"
                    >
                      {qq.label}
                    </button>
                  ))}
                ...
                <form onSubmit={handleSubmit} className="flex items-center gap-2 px-4 py-2 border-t border-[#fcfcfc]/10 bg-[#0c0c0d]">
                  <span className="text-[#b0342e] text-sm">❯</span>
                  <input
                    ref={inputRef}
                    type="text"
                    value={input}
                    ...
                    className="flex-1 bg-transparent text-[#fcfcfc]/90 text-sm outline-none placeholder-[#fcfcfc]/30 font-mono"
                    autoComplete="off"
                    spellCheck={false}
                  />
                </form>
