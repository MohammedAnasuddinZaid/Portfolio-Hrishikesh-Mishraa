...
export const TechStackMarquee: React.FC = () => {
  ...
  return (
    <section id="tech-stack" className="py-24 lg:py-32 relative overflow-hidden bg-[#f5f5f5] border-t border-black/10">
      {/* Infinite Scrolling Marquee Header */}
      <div className="w-full py-6 bg-[#141414] border-y border-black mb-20 relative overflow-hidden">
        <div className="animate-marquee flex items-center gap-8 whitespace-nowrap">
          {[...TECH_STACK, ...TECH_STACK].map((item, idx) => (
            <div
              key={`${item.name}-${idx}`}
              className="flex items-center gap-3 px-6 py-2 border border-white/20 bg-white/5"
            >
              ...
              <span className="text-[9px] text-[#f5f5f5]/60 font-mono">
                {item.experience}
              </span>
            </div>
          ))}
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredStack.map((tech, idx) => (
            <motion.div
              ...
              className="p-6 border border-black/15 bg-white hover:border-black transition-all group shadow-xs"
            >
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-display text-base font-black uppercase text-[#141414] group-hover:text-red-600 transition-colors">
                      {tech.name}
                    </h3>
                    <div className="text-[9px] text-red-600 uppercase tracking-widest font-black">
                      {tech.category}
                    </div>
                  </div>
                </div>
                <span className="px-3 py-1 border border-black/20 bg-[#f5f5f5] text-[9px] font-black uppercase text-[#141414]">
                  {tech.experience}
                </span>
              </div>

              <p className="text-xs text-[#141414]/80 leading-relaxed font-medium">
                {tech.useCase}
              </p>
            </motion.div>
          ))}
