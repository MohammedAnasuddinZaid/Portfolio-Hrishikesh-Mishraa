    ...
    <section className="relative py-32 overflow-hidden bg-white">
      ...
      <div className="max-w-6xl mx-auto px-6">
        <motion.h2
          ...
          className="text-4xl md:text-5xl font-black mb-4 text-[#141414]"
        >
          <span className="text-red-600">❯</span> Testimonials
        </motion.h2>
        <motion.p
          initial={{ clipPath: 'inset(0 100% 0 0)' }}
          whileInView={{ clipPath: 'inset(0 0% 0 0)' }}
            ...
            >
              <div className="flex gap-1 mb-6">
                {Array.from({ length: 5 }).map((_, i) => (
                  <svg key={i} className={`w-5 h-5 ${i < t.rating ? 'text-red-500' : 'text-gray-300'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>
              <blockquote className="text-lg md:text-xl leading-relaxed text-[#141414]/80 mb-8 font-mono">
                "{t.text}"
              </blockquote>
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-red-500 to-red-800 flex items-center justify-center text-white font-black text-lg">
                  {t.name.charAt(0)}
                </div>
                <div>
                  <div className="font-black text-[#141414]">{t.name}</div>
                  <div className="text-sm text-[#141414]/50 font-mono">{t.role}, {t.company}</div>
                </div>
              </div>
            </motion.div>
          </AnimatePresence>
            ...
            {testimonials.map((_, i) => (
              <button
                ...
                className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${
                  i === current ? 'bg-red-500 w-6' : 'bg-[#141414]/20 hover:bg-[#141414]/40'
                }`}
              />
            ))}
          </div>
